...
async function main(){
 ...
 try{
    let fileName = input.question("Enter Image file name : ");
  let character = await fs.readFile(`${fileName}`);

  let binaryArr = [];
  character.forEach(el=>binaryArr.push(el.toString(2).padStart(8, '0')))
